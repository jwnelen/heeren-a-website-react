...
class DaltonsListRow extends Component {
	...
	render(props) {
		let {dalton, index, players, loggedIn} = this.props;
		players = []
			
		return(
		<tr>
      <td>{index}</td>
      <td>{players[dalton.p_earned_id] || '-' }</td>
      <td>{dalton.reason}</td>
      <td>{players[dalton.p_took_id] || '-'}</td>
      <td>{
					dalton.date_earned ? moment(dalton.date_earned).format('dddd DD MMMM YYYY') : ''}</td>
   		<td>{
					dalton.date_took ? moment(dalton.date_took).format('dddd DD MMMM YYYY') : ''}</td>
			{loggedIn && 
			<td>
				<Button variant="transparent" className='p-0' id={dalton.id} onClick={this.handleClick}>
